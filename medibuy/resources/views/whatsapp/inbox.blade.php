...
<style>
  ...
  .wa-title .icon{
    ...
    background: rgba(255,255,255,.20);color:white; flex: 0 0 auto;
  }
  .wa-title h2{
    margin:0;
    font-size:1.15rem;
...
.stat-card h3{
  ...
  .avatar{
    width: 64px; height: 64px; border-radius: 50px; flex: 0 0 auto; overflow:hidden;
...
<div class="wa-shell">
  ...
  <div class="stat-card">
      <span>Sin leer</span>
      <h3>{{ collect($threads)->sum('unread_count') }}</h3>
  </div>

  <div class="stat-card">
      <span>En atención</span>
      <h3>{{ collect($threads)->where('handover',1)->count() }}</h3>
  </div>

  <div class="stat-card">
      <span>Automático</span>
      <h3>{{ collect($threads)->where('handover',0)->count() }}</h3>
  </div>
