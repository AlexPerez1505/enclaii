...
<style>
  ...
  .filters-wrap{ gap:.75rem; display:grid; grid-template-columns: repeat(6, 1fr); }
  @media (max-width: 992px){ .filters-wrap{ grid-template-columns: repeat(3, 1fr); } }
...
$(function(){
  ...
  function qparams(extra={}) {
    ...
    // cache buster para evitar respuestas de navegador
    return {...o, ...extra, _t: Date.now()};
  }
  ...
  function loadTable(){
    $.getJSON(URL_LIST, qparams({page}), function(res){
      ...
      const pp = res.meta.per_page ?? 0;
      $('#pageInfo').text(`Página ${res.meta.page} de ${res.meta.last_page} — ${res.meta.total} movs${pp?` · ${pp}/pág`:''}`);
  ...
  function refreshAll(){
    loadMetrics();
    loadChart();
    // Para no interrumpir navegación, solo refresca la tabla si estás en página 1
    if(page === 1) loadTable();
  }
  ...
  // Auto-refresh cada 20s (no salta de tu página)
