...
.card-head{
  ...
  border-bottom:none;
  background:
    linear-gradient(135deg, #60a5fa 0%, #93c5fd 50%, #d1e2f8 100%);
}
...
/* Body frente */
.card-body{padding:70px 32px 24px}
...
@media (max-width:640px){
  ...
  .avatar-wrap{ width:96px; height:96px; bottom:-46px; transition:.3s ease;
...
@endif
  ...
  <div class="idc-grid">
    ...
    <h2>{{ $user->name }}</h2>
    <p>{{ $user->puesto ?? 'Colaborador' }}</p>
    <div class="stats-grid">
    <div class="stat-card">
        <div class="stat-number">{{ $user->asistencias ?? 0 }}</div>
        <div class="stat-label">Asistencias</div>
    </div>
    

    <div class="stat-card">
        <div class="stat-number">{{ $user->vacaciones_disponibles ?? 0 }}</div>
        <div class="stat-label">Vacaciones</div>
    </div>

    <div class="stat-card">
        <div class="stat-number">{{ $user->retardos ?? 0 }}</div>
    ...
    <a href="{{ route('perfil.edit') }}" class="quick-btn">
        Editar Perfil
    </a>

    <a href="{{ route('auth.change-password') }}" class="quick-btn">
        Contraseña
    </a>

    <a href="{{ route('mi-historial') }}" class="quick-btn">
        Historial
    </a>
</div>
            ...
            <p class="help">Tip: toca la tarjeta para ver el reverso.</p>
          </section>
        </article>
