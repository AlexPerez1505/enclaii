        ...
        <table class="w-100">
            ...
            @foreach($venta->productos as $vp)
                @php
                    ...
                    $subLinea   = (float)($vp->subtotal ?? 0);
                    $precioUnit = ($vp->cantidad > 0)
                    ? $subLinea / $vp->cantidad
                    : (float)($vp->precio_unitario ?? 0);

                    // ✅ Si subtotal 0 o menor: NO mostrar nada en P.Unit ni Subtotal
                    $ocultarLinea = ($subLinea <= 0);
                @endphp
                <tr>
                    <td>
                        <div>{{ $nombreLargo ?: '—' }}</div>
