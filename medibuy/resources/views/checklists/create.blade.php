...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function showStep(s) {
            ...
            console.log('Cambiando a paso:', s);
        }
        showStep(step);

        document.getElementById('btn-next').onclick = function(e){
            e.preventDefault();
            console.log('Click siguiente, paso actual:', step);
            if(step < totalSteps) {
                step++;
                showStep(step);
            } else {
                // Antes de submit, log todos los datos del form
                let data = {};
        ...
        document.getElementById('btn-back').onclick = function(e){
            e.preventDefault();
            console.log('Click atrás, paso actual:', step);
            if(step > 1) {
                step--;
                showStep(step);
            }
...
<div class="container mt-4">
    <div class="main-card">
        ...
        <form id="wizard-form" action="{{ route('checklists.store') }}" method="POST" autocomplete="off">
            @csrf
    ...
    <select name="user_id" class="form-control" required>
        ...
        @endforeach
            ...
            <div class="wizard-pane" style="display:none;">
                <div class="mb-3">
                    <label>Confirme la información antes de guardar</label>
