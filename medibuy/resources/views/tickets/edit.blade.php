...
.edit-wrap{
  max-width:940px;               /* móvil cómodo “como antes” */
  ...
  <div class="panel">
    <div class="panel-head">
      <div class="hgroup">
        ...
        <p>Actualiza la información y gestiona el acceso.</p>
      </div>
      <a href="{{ route('tickets.show',$ticket) }}" class="back-link" title="Volver">
    ...
    <form class="form" action="{{ route('tickets.update',$ticket) }}" method="POST">
      ...
      <div class="grid">
        ...
        <div class="compact @error('title') is-invalid @enderror">
          <div class="field">
            <input type="text" name="title" id="f-title"
                   value="{{ old('title',$ticket->title) }}" placeholder=" " required maxlength="180">
        ...
        <div class="compact @error('priority') is-invalid @enderror">
          <div class="select-wrap">
            ...
            <select id="f-priority" name="priority" required>
              @php $prio = old('priority',$ticket->priority); @endphp
              ...
              @if(in_array('urgent', ['urgent'])){{-- por si tu enum lo incluye --}}
                <option value="urgent" @selected($prio==='urgent')>Urgente</option>
              @endif
            </select>
          </div>
        ...
        {{-- Visibilidad (DB: private / shared / public) --}}
        <div class="compact @error('visibility') is-invalid @enderror">
          <div class="select-wrap">
            <label for="f-visibility">Visibilidad *</label>
            @php $vis = old('visibility',$ticket->visibility); @endphp
            <select id="f-visibility" name="visibility" required>
              <option value="private" @selected($vis==='private')>Privado (sólo creador)</option>
              <option value="shared"  @selected($vis==='shared')>Seleccionados</option>
              <option value="public"  @selected($vis==='public')>Todos</option>
            </select>
          </div>
          @error('visibility')<div class="error">{{ $message }}</div>@enderror
          <div class="small" style="color:var(--muted);margin-top:6px;">
            <strong>Nota:</strong> los usuarios en “Acceso (watchers)” sólo aplican cuando la visibilidad es <em>Seleccionados</em>.
          </div>
        </div>

        {{-- Asignado (uno) --}}
        <div class="compact @error('assignee_id') is-invalid @enderror">
          <div class="select-wrap">
            <label for="f-assignee">Asignado a</label>
            <select id="f-assignee" name="assignee_id">
              <option value="">— Ninguno —</option>
              @php $assigneeId = old('assignee_id',$ticket->assignee_id); @endphp
              @foreach($users as $u)
        ...
        {{-- Acceso (watchers) con chips --}}
        <div class="compact @error('watchers') is-invalid @enderror" style="position:relative;">
          <div class="ms" id="watchersMS">
            <label>Acceso (watchers)</label>
            <div class="ms-list">
              <div class="ms-box" id="watchersBox">
                {{-- Chips se renderizan aquí --}}
                <input type="text" id="watchersInput" class="ms-input" placeholder="Escribe para buscar…">
              </div>
              <div class="ms-dropdown" id="watchersDropdown"></div>
            </div>
          </div>
          @error('watchers')<div class="error">{{ $message }}</div>@enderror
          {{-- hidden inputs --}}
          <div id="watchersHidden"></div>
        </div>

        {{-- Descripción (fila completa) --}}
        <div style="grid-column:1 / -1" class="@error('description') is-invalid @enderror">
          <div class="field">
            <textarea name="description" id="f-body" placeholder=" ">{{ old('description',$ticket->description) }}</textarea>
...
<script>
(function(){
  /* ===== Fuente de usuarios para chips (id, name) ===== */
  const AVAILABLE = @json(
    collect($users ?? [])->map(fn($u)=>['id'=>$u->id,'name'=>$u->name])->values()
  );

  // Preselección: watchers actuales o old('watchers')
  @php
    $oldWatchers = collect(old('watchers', $ticket->watchers->pluck('id')->all()));
    $pref = $oldWatchers->map(fn($id)=> (object)['id'=>$id, 'name'=>optional(($users ?? collect())->firstWhere('id',$id))->name])
                        ->filter(fn($u)=>$u->name);
  @endphp
  const prefill = @json($pref->values());

  const selected = new Map(); // id -> name
  prefill.forEach(u => selected.set(String(u.id), u.name));

  const box = document.getElementById('watchersBox');
  const input = document.getElementById('watchersInput');
  const dd = document.getElementById('watchersDropdown');
  const hiddenWrap = document.getElementById('watchersHidden');

  function renderHidden(){
    hiddenWrap.innerHTML = '';
    for(const [id] of selected){
      const h = document.createElement('input');
      h.type = 'hidden'; h.name = 'watchers[]'; h.value = id;
