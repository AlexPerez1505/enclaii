...
<div class="view">
  ...
  <div class="head">
    <div>
      <h2 class="title">Nuevo Activo</h2>
      <div class="sub">Captura activos fijos y consumibles con una vista más limpia y profesional.</div>
    </div>
  </div>
  ...
  <form id="assetForm" method="POST" action="{{ route('assets.store') }}" enctype="multipart/form-data" class="cardx">
    @csrf
      ...
      <div>
        <div class="panel-lite">
          ...
          <div class="form-grid two">
            <div class="fg">
              <select class="ctl" name="type" id="type" required placeholder=" ">
                ...
                <option value="activo_fijo" {{ old('type') === 'activo_fijo' ? 'selected' : '' }}>Activo Fijo</option>
                <option value="consumible" {{ old('type') === 'consumible' ? 'selected' : '' }}>Consumible</option>
              </select>
              <label>Tipo *</label>
            </div>
            ...
            <div>
              <div class="fg">
                <select class="ctl" name="inventory_category_id" id="inventory_category_id" required placeholder=" ">
                  <option value="" disabled {{ old('inventory_category_id') ? '' : 'selected' }}></option>
                  @foreach($categories as $c)
                    <option value="{{ $c->id }}" {{ (string)old('inventory_category_id') === (string)$c->id ? 'selected' : '' }}>
                      {{ $c->name }}
                    </option>
                  @endforeach
                </select>
          ...
          <div class="form-grid" style="margin-top:14px;">
            <div class="fg">
              <input class="ctl" name="name" required placeholder=" " value="{{ old('name') }}">
              <label>Nombre *</label>
            </div>
          </div>
          ...
          <div class="form-grid two" style="margin-top:14px;">
            <div class="fg">
              <input class="ctl" name="unit" placeholder=" " value="{{ old('unit', 'piezas') }}">
              <label>Unidad</label>
            </div>

            <div class="fg">
              <input class="ctl" type="number" min="0" name="stock" value="{{ old('stock', 0) }}" placeholder=" ">
              <label>Stock actual *</label>
            </div>

            <div class="fg" style="grid-column:1 / -1;">
              <input class="ctl" name="location" placeholder=" " value="{{ old('location') }}">
          ...
          <div id="fixedFields" class="form-grid two">
            <div class="fg">
              <select class="ctl" name="asset_status" id="asset_status" placeholder=" ">
                <option value="" disabled {{ old('asset_status') ? '' : 'selected' }}></option>
                <option value="disponible" {{ old('asset_status') === 'disponible' ? 'selected' : '' }}>Disponible</option>
                <option value="asignado" {{ old('asset_status') === 'asignado' ? 'selected' : '' }}>Asignado</option>
                <option value="en_reparacion" {{ old('asset_status') === 'en_reparacion' ? 'selected' : '' }}>En reparación</option>
                <option value="dado_de_baja" {{ old('asset_status') === 'dado_de_baja' ? 'selected' : '' }}>Dado de baja</option>
              </select>
              <label>Estado</label>
            </div>
            ...
            <div class="fg">
              <select class="ctl" name="condition" id="condition" placeholder=" ">
                ...
                <option value="bueno" {{ old('condition') === 'bueno' ? 'selected' : '' }}>Bueno</option>
                <option value="regular" {{ old('condition') === 'regular' ? 'selected' : '' }}>Regular</option>
                <option value="malo" {{ old('condition') === 'malo' ? 'selected' : '' }}>Malo</option>
              </select>
              <label>Condición</label>
            </div>

            <div class="fg">
              <input class="ctl" name="brand" placeholder=" " value="{{ old('brand') }}">
              <label>Marca</label>
            </div>

            <div class="fg">
              <input class="ctl" name="model" placeholder=" " value="{{ old('model') }}">
              <label>Modelo</label>
            </div>

            <div class="fg" style="grid-column:1 / -1;">
              <input class="ctl" name="serial_number" placeholder=" " value="{{ old('serial_number') }}">
          ...
          <div id="consumableFields" class="form-grid two">
            <div class="fg">
              <input class="ctl" type="number" min="0" name="stock_min" value="{{ old('stock_min', 0) }}" placeholder=" ">
              <label>Stock mínimo *</label>
            </div>

            <div class="fg">
              <input class="ctl" type="number" min="0" name="stock_max" value="{{ old('stock_max', 0) }}" placeholder=" ">
          ...
          <div class="form-grid" style="margin-top:14px;">
            <div class="fg">
              <textarea class="ctl" name="notes" rows="4" placeholder=" ">{{ old('notes') }}</textarea>
              <label>Notas</label>
            </div>
          </div>
