              ...
              <tr>
                ...
                <td data-label="Acciones" style="text-align:right">
                  <div class="td-actions">
                    ...
                    <form action="{{ route('prestamos.destroy',$p->id) }}" method="POST" style="display:inline"
...
// ✅ Eliminar con PIN (SweetAlert2) + confirmación
async function confirmDelete(e, form){
  e.preventDefault();

  const askPin = await Swal.fire({
    title: 'PIN de aprobación',
    text: 'Para eliminar este préstamo, captura tu PIN (6 dígitos).',
    input: 'password',
    inputPlaceholder: '******',
    inputAttributes: {
      maxlength: 6,
    ...
    preConfirm: (val) => {
      const pin = (val || '').trim();
      if(!/^\d{6}$/.test(pin)){
        Swal.showValidationMessage('El PIN debe tener 6 dígitos numéricos.');
        return false;
      }
      return pin;
    }
  ...
  if(!askPin.isConfirmed) return false;
  ...
  if(!sure.isConfirmed) return false;

  form.querySelector('input[name="pin"]').value = askPin.value;
  form.submit();
  return false;
}
</script>
