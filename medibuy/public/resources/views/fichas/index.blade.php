...
<style>
    ...
    /* ===== BOTÓN “AGREGAR FICHA” ESTILO Btn ===== */
    .add-ficha-btn {
        display: flex;
        align-items: center;
        justify-content: flex-start;
    ...
    /* === MODO CELULAR: BOTÓN FLOTANTE INFERIOR IZQUIERDO === */
/* === MODO CELULAR: BOTÓN FLOTANTE INFERIOR DERECHA === */
@media (max-width: 640px) {
    .add-ficha-btn {
        position: fixed;
        bottom: 18px;
        right: 18px;   /* 👉 antes estaba left: 18px */
        left: auto;    /* aseguramos que no se quede el left anterior */
        width: 56px;
        height: 56px;
        border-radius: 999px;
                ...
                <tbody id="fichasTableBody">
                    @forelse ($fichas as $ficha)
                        <tr class="ft-row">
                            ...
                            <td class="ft-download-cell" data-label="Archivo">
                                {{-- BOTÓN DOWNLOAD NEGRO ICON-ONLY --}}
                                <a href="{{ route('fichas.download', $ficha) }}"
                                   class="button"
                                   title="Descargar ficha">
                                    <span class="sr-only">Descargar ficha</span>
                                    <div>
                    ...
                    @empty
                        <tr>
                            <td colspan="3" style="padding: 1rem; text-align: center; color: var(--ft-text-muted);">
