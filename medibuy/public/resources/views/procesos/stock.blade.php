...
/* Buttons */
.btn-ghost{ border:1px solid var(--line); background:#fff; color:#111827; border-radius:12px; padding:.7rem 1rem; font-weight:800 }
.btn-primary-soft{
...
<div class="wrap" x-data="StockUI()" x-init="initLote()">
  <div class="headcard d-flex align-items-center justify-content-between flex-wrap gap-2">
    <div>
      <div class="title h4 mb-1">Stock — Equipo #{{ $equipoId }}</div>
      <div class="subtle">Carga evidencias, describe el estado y completa el checklist de ingreso.</div>
    </div>
    <span class="badge-dot"><span class="dot"></span> Proceso activo</span>
  </div>

  <form id="frmStock" class="mt-3" action="{{ $saveUrl }}" method="POST" enctype="multipart/form-data" @submit.prevent="submit">
    @csrf
    ...
    <div class="card-soft mt-3">
      <div class="hd"><strong>Descripción del ingreso</strong></div>
      <div class="bd">
        <textarea
          class="form-control"
          name="descripcion_proceso"
          rows="4"
          placeholder="Describe condición general, accesorios incluidos, ubicación de almacén y cualquier hallazgo relevante."
          required
        ></textarea>
        ...
        <div class="form-check mt-3" x-show="isLoteMaster">
          <input
            ...
            type="checkbox"
            id="aplicar_lote_stock"
            name="aplicar_lote"
            value="1"
            x-model="aplicarLote"
          >
          <label class="form-check-label" for="aplicar_lote_stock">
            Aplicar este proceso de stock a todos los equipos del lote
          </label>
        ...
        <div class="grid-tiles">
          ...
          <label class="tile">
            ...
            <div class="hint" x-show="!images[0]">
              <div class="mb-1">Foto frontal</div>
              <small>PNG/JPG</small>
            </div>
          ...
          <label class="tile">
            ...
            <div class="hint" x-show="!images[2]">
              <div class="mb-1">Serie / etiquetas</div>
              <small>PNG/JPG</small>
            </div>
        ...
        <div class="grid-tiles mt-3" style="grid-template-columns:1fr;">
          ...
          <label class="tile">
            ...
            <div class="hint" x-show="!videoUrl">
              <div class="mb-1">Video de funcionamiento (opcional)</div>
    ...
    <div class="card-soft mt-3">
      <div class="hd">
        <strong>Checklist de ingreso a Stock</strong>
      </div>
...
function StockUI(){
  return {
    ...
    initChecklist(){
      // Presets generales para equipo médico (stock)
      this.groups = [
        {
          name:'Integridad física',
          items:[
            {label:'Gabinete sin golpes ni fisuras', hint:'Inspección visual 360°', done:false, note:''},
            {label:'Pantallas/lentes sin rayas o píxeles muertos', hint:'Aplica para monitores, ultrasonidos, torres', done:false, note:''},
            {label:'Conectores y puertos sin daño', hint:'Alimentación, video, señal, gases', done:false, note:''},
            {label:'Cables de poder en buen estado', hint:'Sin pelones ni parches', done:false, note:''},
          ]
        },
        {
          name:'Accesorios y contenido',
          items:[
            {label:'Accesorios incluidos completos', hint:'Pedales, sondas, transductores, mandos, fibras, mangueras', done:false, note:''},
            {label:'Manual/archivos disponibles', hint:'PDF o físico (si aplica)', done:false, note:''},
            {label:'Carro/base/soportes (si aplica)', hint:'Frenos y ruedas operativos', done:false, note:''},
          ]
        },
        {
          name:'Pruebas rápidas',
          items:[
            {label:'Enciende correctamente', hint:'Arranque básico', done:false, note:''},
            {label:'Señal de video / imagen básica', hint:'Endoscopia/laparo/ultrasonido/monitor', done:false, note:''},
            {label:'Puertos principales probados', hint:'Video/USB/Red/Footswitch', done:false, note:''},
            {label:'Carga/batería (si aplica)', hint:'Carga inicial y retención', done:false, note:''},
          ]
        },
        {
          name:'Logística de almacén',
          items:[
            {label:'Limpieza y desinfección', hint:'Equipo listo para almacenamiento', done:false, note:''},
            {label:'Etiqueta y código de barras colocados', hint:'Número de serie visible', done:false, note:''},
            {label:'Ubicación en almacén registrada', hint:'Anaquel / estante / zona', done:false, note:''},
            {label:'Embalaje listo para envío', hint:'Protecciones y sujeción', done:false, note:''},
            {label:'Fotos y video cargados', hint:'Evidencias en el sistema', done:false, note:''},
          ]
        }
      ];
      this.syncChecklist();
    ...
    submit(){
      ...
      const form = document.getElementById('frmStock');
          ...
          xhr.onload=()=>{
            if(xhr.status===201 || xhr.status===200){
              Swal.fire({
                ...
                text:'Proceso de stock guardado correctamente.',
                width:'420px',
                background:'#ffffff',
                customClass:{
                  popup:'swal2-elegant',
